...
func TestMSG91WidgetAccessTokenVerifier_UsesPhoneFromVerifiedResponse(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"type":"success","data":{"identifier":"919876543210"}}`))
	}))
	defer server.Close()
	...
	if err := verifier.VerifyAccessToken(context.Background(), accessToken, "+919876543210"); err != nil {
		t.Fatalf("expected response identifier to be accepted, got %v", err)
	}
}
